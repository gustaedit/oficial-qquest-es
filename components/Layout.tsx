...
import { Target, BarChart3, Shield, Package, LogOut, Sun, Moon, Crosshair, Wifi, WifiOff, User } from 'lucide-react';
...
export const Layout: React.FC<LayoutProps> = ({
  ...
  useEffect(() => {
    ...
    // Obtém o usuário atual para exibir no avatar
    const getUser = async () => {
      const { data: { user } } = await supabase.auth.getUser();
      if (user) setUserEmail(user.email ?? 'BA');
    };
    getUser();
  }, []);
  ...
  return (
    <div className="flex h-screen bg-[#E9ECEF] dark:bg-[#0A0A0A] text-gray-900 dark:text-white overflow-hidden flex-col md:flex-row transition-colors duration-300">
      ...
      <nav className="md:hidden fixed bottom-4 left-4 right-4 h-16 bg-white/95 dark:bg-black/90 backdrop-blur-xl border border-gray-300 dark:border-white/10 z-50 flex items-center justify-around px-2 rounded-3xl shadow-2xl">
        ...
        <MobileIcon active={currentPage === 'profile'} onClick={() => setCurrentPage('profile')} icon={<User />} label="Perfil" />
      </nav>

      {/* ── CONTEÚDO PRINCIPAL ── */}
      <main className="flex-1 flex flex-col relative mb-24 md:mb-0 overflow-hidden">
        <header className="h-16 md:h-20 px-4 md:px-10 flex items-center justify-between border-b border-gray-300 dark:border-white/5 bg-[#F1F5F9]/80 dark:bg-black/50 backdrop-blur-md sticky top-0 z-40">
          <h2 className="text-[10px] md:text-sm font-bold uppercase tracking-[0.3em] text-primary italic">
            {PAGE_LABELS[currentPage] ?? currentPage}
          </h2>

          <div className="flex items-center gap-4 md:gap-8">
            <div className={`flex items-center gap-2 px-3 py-1.5 rounded-full border ${connectionStatus === 'online' ? 'border-emerald-500/30 text-emerald-500' : 'border-red-500/30 text-red-500'}`}>
              {connectionStatus === 'online' ? <Wifi className="w-3 h-3" /> : <WifiOff className="w-3 h-3" />}
              <span className="text-[8px] font-black uppercase tracking-widest hidden sm:inline">{connectionStatus === 'online' ? 'Sincronizado' : 'Offline'}</span>
            </div>
            ...
            {/* Avatar Clicável corrigido */}
            <button 
              onClick={() => setCurrentPage('profile')}
              className={`w-10 h-10 rounded-2xl border-2 transition-all flex items-center justify-center shadow-sm ${
                currentPage === 'profile' ? 'border-primary bg-primary/10 scale-105' : 'border-primary/30 bg-gray-300 dark:bg-white/5 hover:border-primary/60'
              }`}
            >
              <span className="font-bold text-xs text-primary">
                {userEmail ? userEmail.substring(0, 2).toUpperCase() : 'BA'}
              </span>
            </button>
          </div>
        </header>
...
const SidebarIcon = ({ active, onClick, icon, label }: any) => (
  <button onClick={onClick} className={`flex flex-col items-center gap-2 group transition-all ${active ? 'text-primary' : 'text-gray-600 dark:text-gray-500 hover:text-gray-900 dark:hover:text-white'}`}>
    ...
    <span className="text-[10px] font-bold uppercase tracking-widest scale-0 group-hover:scale-100 transition-all origin-top">{label}</span>
  </button>
);

const MobileIcon = ({ active, onClick, icon, label }: any) => (
  <button onClick={onClick} className={`flex flex-col items-center justify-center w-full h-full gap-1 transition-all ${active ? 'text-primary scale-110' : 'text-gray-600 dark:text-gray-500'}`}>
