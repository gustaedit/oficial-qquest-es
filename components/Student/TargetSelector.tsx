...
import { Shield, Target, ChevronRight, Zap, SlidersHorizontal, BookOpen, Crosshair, Hash, Play, Bookmark } from 'lucide-react';
...
export const TargetSelector: React.FC<TargetSelectorProps> = ({ tags, questions, onSelect, savedScenarios = [] }) => {
      ...
      <div className="max-w-5xl mx-auto py-4 animate-in fade-in zoom-in-95 duration-500">
        ...
        {savedScenarios.length > 0 && (
          <div className="mb-10 animate-in slide-in-from-top-4 duration-700">
            <h4 className="text-[10px] font-black uppercase text-gray-500 dark:text-white/20 mb-5 px-2 flex items-center gap-3 tracking-[0.2em]">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
              {savedScenarios.map((s: any) => (
                <button
                  ...
                  className="group p-5 bg-white dark:bg-white/[0.02] border border-gray-300 dark:border-white/5 rounded-[2rem] hover:border-primary/40 transition-all text-left"
                >
                  <div className="flex items-center justify-between mb-3">
                    <div className="p-2 bg-primary/10 rounded-xl group-hover:bg-primary group-hover:text-black transition-all">
                      <Play className="w-4 h-4 fill-current" />
                    </div>
                    <span className="text-[8px] font-black uppercase text-white/20">{s.filters?.limit || '20'} Qs</span>
                  </div>
                  <h5 className="text-sm font-black uppercase italic text-gray-900 dark:text-white truncate">{s.title}</h5>
                  <p className="text-[8px] font-bold uppercase text-gray-500 dark:text-white/20 mt-1 truncate">
                    {s.filters?.disciplines?.join(' · ') || 'Filtros Gerais'}
                  </p>
                </button>
              ))}
            </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-5 mb-5">
          <button onClick={() => setMode('discipline')} className="group p-7 bg-white dark:bg-white/[0.03] border-2 border-gray-300 dark:border-white/10 rounded-[2.5rem] hover:border-primary transition-all text-left flex flex-col gap-4">
            <div className="p-4 bg-primary/10 rounded-2xl w-fit group-hover:scale-110 transition-transform"><BookOpen className="w-7 h-7 text-primary" /></div>
            <div>
              <h3 className="text-xl font-black uppercase italic text-gray-900 dark:text-white group-hover:text-primary transition-colors">Por Disciplina</h3>
          ...
          <button onClick={() => setMode('career')} className="group p-7 bg-white dark:bg-white/[0.03] border-2 border-gray-300 dark:border-white/10 rounded-[2.5rem] hover:border-blue-500 transition-all text-left flex flex-col gap-4">
            <div className="p-4 bg-blue-500/10 rounded-2xl w-fit group-hover:scale-110 transition-transform"><Target className="w-7 h-7 text-blue-500" /></div>
            <div>
              <h3 className="text-xl font-black uppercase italic text-gray-900 dark:text-white group-hover:text-blue-500 transition-colors">Por Carreira</h3>
          ...
          <button onClick={() => onSelect({ isCustom: true })} className="group p-7 bg-white dark:bg-white/[0.03] border-2 border-gray-300 dark:border-white/10 rounded-[2.5rem] hover:border-cyan-500 transition-all text-left flex flex-col gap-4">
            <div className="p-4 bg-cyan-500/10 rounded-2xl w-fit group-hover:scale-110 transition-transform"><SlidersHorizontal className="w-7 h-7 text-cyan-500" /></div>
            <div>
              <h3 className="text-xl font-black uppercase italic text-gray-900 dark:text-white group-hover:text-cyan-500 transition-colors">Filtro Avançado</h3>
        ...
        <button onClick={() => onSelect({})} className="w-full group flex items-center justify-between px-7 py-4 border-2 border-dashed border-gray-300 dark:border-white/10 rounded-3xl hover:border-primary/40 transition-all">
    ...
    return (
      <div className="max-w-5xl mx-auto py-4 animate-in fade-in duration-400">
        <div className="flex items-center gap-4 mb-8">
          <button onClick={() => { setMode('home'); setSelectedDiscipline(''); }} className="px-5 py-2.5 bg-gray-200 dark:bg-white/5 rounded-2xl text-[10px] font-black uppercase text-gray-600 dark:text-white/40 hover:text-primary transition-all">← Voltar</button>
          <h2 className="text-2xl font-black uppercase italic text-gray-900 dark:text-white">{selectedDiscipline || 'Escolha a Disciplina'}</h2>
        </div>
        ...
        {!selectedDiscipline ? (
              ...
              return (
                ...
                >
                  <div className="flex items-start justify-between">
                    ...
                    <span className={`text-[9px] font-black uppercase tracking-widest px-2.5 py-1 rounded-full ${meta.bg} ${meta.color}`}>{disciplineCount[disc] || 0} questões</span>
                  </div>
          ...
          <div className="animate-in fade-in duration-300 space-y-4">
            <button onClick={() => onSelect({ discipline: selectedDiscipline })} className="w-full flex items-center justify-between px-7 py-5 bg-primary text-black rounded-3xl font-black uppercase text-[11px] tracking-widest hover:brightness-105 transition-all">
              <div className="flex items-center gap-3"><Zap className="w-5 h-5 fill-current" /> Toda a disciplina</div>
              <Play className="w-5 h-5 fill-current" />
            </button>
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
              {availableTopics.map(topic => (
                <button key={topic} onClick={() => onSelect({ discipline: selectedDiscipline, topic })} className="flex items-center justify-between p-4 bg-white dark:bg-white/[0.03] border-2 border-gray-200 dark:border-white/10 rounded-2xl hover:border-primary transition-all">
                  <span className="text-sm font-bold text-gray-800 dark:text-white/80">{topic}</span>
  ...
  if (mode === 'career') {
    return (
      <div className="max-w-5xl mx-auto py-4 animate-in fade-in duration-400">
        <button onClick={() => setMode('home')} className="px-5 py-2.5 bg-gray-200 dark:bg-white/5 rounded-2xl text-[10px] font-black uppercase text-gray-600 dark:text-white/40 mb-8">← Voltar</button>
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-5">
          {careerOptions.map(career => (
            <button key={career.id} onClick={() => onSelect({ contestClass: career.id })} className="group p-7 bg-white dark:bg-white/[0.03] border-2 border-gray-300 dark:border-white/10 rounded-[2.5rem] hover:border-primary transition-all text-left flex flex-col gap-4">
              <div className="p-3 bg-primary/10 rounded-2xl group-hover:scale-110 transition-transform"><Crosshair className="w-6 h-6 text-primary" /></div>
              <h3 className="text-base font-black uppercase italic text-gray-900 dark:text-white">{career.label}</h3>
              <p className="text-[9px] font-bold uppercase text-gray-500 dark:text-white/30">{career.desc}</p>
            </button>
          ))}
        </div>
      </div>
