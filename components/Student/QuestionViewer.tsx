...
export const QuestionViewer: React.FC<QuestionViewerProps> = ({
  ...
  // ── ESTADO DE PERSISTÊNCIA DAS RESPOSTAS ──
  // Mapeia o ID da questão para a opção selecionada
  ...
  // Variáveis auxiliares para controle de estado por questão
  ...
  const handleConfirm = () => {
    ...
    // Persiste a resposta no mapa local
    setAnswers(prev => ({
        ...prev,
  ...
  return (
    <div className="max-w-4xl mx-auto py-2 animate-in fade-in duration-500">
                    ...
                    {!hasAnswered && (
                      <button
                        ...
                        className={`absolute right-3 top-1/2 -translate-y-1/2 w-9 h-9 flex items-center justify-center rounded-xl transition-all duration-200 ${
                          isCut ? 'bg-amber-400/20 text-amber-500 opacity-100' : 'opacity-0 group-hover/opt:opacity-70 bg-gray-200/80 dark:bg-white/10'
                        }`}
                      >
      ...
      <div className="flex flex-col sm:flex-row items-center gap-4">
        ...
        {!hasAnswered ? (
          <button
            ...
            className={`flex-1 py-8 ${currentQuestion.isAI ? 'bg-cyan-500 shadow-cyan-500/20' : 'bg-primary shadow-primary/20'} text-black font-black uppercase tracking-[0.4em] text-sm rounded-[2.5rem] shadow-2xl hover:scale-[1.02] transition-all disabled:opacity-20 flex items-center justify-center gap-4 active:scale-95`}
          >
          ...
          <div className="flex-1 flex flex-col sm:flex-row gap-4">
            ...
            <button
              ...
              className="flex-1 px-8 py-6 border-2 border-blue-500/40 text-blue-600 dark:text-blue-400 font-black uppercase tracking-widest text-[10px] rounded-[2rem] flex items-center justify-center gap-3"
            >
              <MessageSquare className="w-5 h-5" /> Comentários {comments.length > 0 && <span className="bg-blue-500 text-white text-[9px] rounded-full px-2 py-0.5 ml-1">{comments.length}</span>}
            </button>
            <button
              onClick={nextQuestion}
              className={`flex-[1.5] py-6 ${currentQuestion.isAI ? 'bg-cyan-500' : 'bg-primary'} text-black font-black uppercase tracking-[0.3em] text-[10px] rounded-[2rem] flex items-center justify-center gap-3`}
            >
      ...
      {/* BLOCOS DE FUNDAMENTAÇÃO E COMENTÁRIOS IGUAIS AO ORIGINAL... */}
      {hasAnswered && showSolution && (
        <div className="mt-8 animate-in slide-in-from-top-4 duration-500">
          <div className="p-10 md:p-14 bg-emerald-500/5 border border-emerald-500/20 rounded-[3rem] space-y-8">
            <div className="flex items-center gap-4">
              <div className="p-3 bg-emerald-500 text-black rounded-2xl"><BookOpen className="w-6 h-6" /></div>
            ...
            <p className="text-lg md:text-xl font-bold italic leading-relaxed text-gray-800 dark:text-white/80 border-l-4 border-emerald-500 pl-8">"{currentQuestion.comment}"</p>
          </div>
        </div>
      )}
      ...
      {hasAnswered && showComments && (
        <div className="mt-8 animate-in slide-in-from-top-4 duration-500 pb-20">
          <div className="p-8 md:p-12 bg-blue-500/5 border border-blue-500/20 rounded-[3rem] space-y-8">
            <div className="flex items-center gap-4">
              <div className="p-3 bg-blue-500 text-white rounded-2xl"><MessageSquare className="w-6 h-6" /></div>
              <h5 className="text-sm font-black uppercase text-blue-600 dark:text-blue-400">Comentários da Turma</h5>
            </div>
            {/* TEXTAREA E LISTA DE COMENTÁRIOS... */}
          </div>
        </div>
      )}
